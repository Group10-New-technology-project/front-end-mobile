...
import { View, StyleSheet, Dimensions, Image, Text, Button, TextInput, TouchableOpacity } from 'react-native';
      ...
      <View>
        <View style={{ justifyContent: 'center', alignContent: 'center' }}>
          ...
          <View style={{ width: '100%', alignItems: 'center', marginTop: 10 }}>
            <View style={{ marginTop: 10, alignItems: 'center', flexDirection: 'row', height: 50, width: '90%', justifyContent: 'space-between', alignItems: 'center', borderWidth: 2, borderColor: "#3D3D3D", borderRadius: 8 }}>
              {!selectedDate && (
                <Text style={{ fontSize: 20, fontWeight: 500, color: "#343434", marginLeft: 20 }}>Sinh Nhật</Text>
              )}
              <View >
                <DateTimePickerModal isVisible={isDatePickerVisible} mode="date" onConfirm={handleConfirm} onCancel={hideDatePicker} />
                {selectedDate && <Text style={{ fontSize: 20, fontWeight: 500, color: "#343434", marginLeft: 20 }}>{selectedDate.toLocaleDateString()}</Text>}
              </View>
