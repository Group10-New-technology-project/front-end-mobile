...
export default function TaoMatKhau({ navigation, route }) {
  ...
  const hanlde_chonTen = async () => {
    try {
      const response = await fetch('http://192.168.3.120:3000/api/v1/users/sinup', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ username: SoDienThoai, password }),
      });
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.title_password}>
        <Text style={styles.text_3}>• Mật khẩu của bạn phải chứa ít nhất 8 ký tự, bao gồm ít nhất 1 chữ cái và 1 số.</Text>
